...
class GitHubSearchControllerTest {
  ...
  void returnsScoredRepositoriesForValidRequest() throws Exception {
    var request = new GitHubSearchRequest("java", Instant.now().minusSeconds(3600), 1, 2);
    ...
    when(
        searchService.searchAndScoreRepositories(request.language(), request.earliestCreationDate(),
