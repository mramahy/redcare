...
/**
 * GitHubSearchController is a REST controller that handles requests for searching and scoring
 * GitHub repositories. It provides an endpoint to retrieve scored repositories based on search
 * criteria.
 */
@RestController
public class GitHubSearchController {
  ...
  /**
   * Handles GET requests to the /api/scored-search endpoint. Searches and scores GitHub
   * repositories based on the provided request parameters.
   *
   * @param request the search request containing language, creation date, pagination, and other
   *                criteria
   * @return a ResponseEntity containing a list of scored repositories
   */
  @GetMapping("/api/scored-search")
  public ResponseEntity<List<ScoredRepository>> getScoredRepos(
      @Valid @ModelAttribute GitHubSearchRequest request) {
